...
CY_ISR(TIMER_INTERRUPT_Interrupt)
{
    ...
    /* PSoC3 ES1, ES2 RTC ISR PATCH  */ 
        ...
        #if((CYDEV_CHIP_REVISION_USED <= CYDEV_CHIP_REVISION_3A_ES2) && (TIMER_INTERRUPT__ES2_PATCH ))      
            TIMER_INTERRUPT_ISR_PATCH();
...
{
    CY_SET_REG16(TIMER_INTERRUPT_INTC_VECTOR, (uint16) address);
}
...
{
    return (cyisraddress) CY_GET_REG16(TIMER_INTERRUPT_INTC_VECTOR);
}
...
{
    *TIMER_INTERRUPT_INTC_PRIOR = priority << 5;
}
...
uint8 TIMER_INTERRUPT_GetPriority(void) 
{
    ...
    priority = *TIMER_INTERRUPT_INTC_PRIOR >> 5;
...
uint8 TIMER_INTERRUPT_GetState(void) 
{
    ...
    return ((*TIMER_INTERRUPT_INTC_SET_EN & (uint8)TIMER_INTERRUPT__INTC_MASK) != 0u) ? 1u:0u;
}
